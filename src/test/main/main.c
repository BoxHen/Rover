  ...
  while(1) {
    ...
    switch(decision){
      ...
      case 3: // obstacle turn for left wall start
          turnRight(); _delay_ms(900);
          while(distanceL <= 15){ forward(); distanceL = distanceLeft();} // move forward if you are against the wall of the obstacle
          turnLeft(); _delay_ms(900);
          while(distanceL >= 15){ forward(); distanceL = distanceLeft();} // after turn we will not be aginst the wall. move forward until you are
          while(distanceL <= 15){ forward(); distanceL = distanceLeft();} // move forward if you are against the wall of the obstacle
          turnLeft(); _delay_ms(900);
          while(distanceF >= 15){ forward(); distanceF = distanceFront();} // move forward until front sensor sees a wall
          turnRight(); _delay_ms(900);
          decision = 5;
          break;
      case 4: // obstacle turn right
          ...
          while(distanceL <= 15){ forward(); distanceL = distanceLeft();} // move forward if you are against the wall of the obstacle
          turnRight(); _delay_ms(900);
          while(distanceR >= 15){ forward(); distanceR = distanceRight();} // after turn we will not be aginst the wall. move forward until you are
          while(distanceR <= 15){ forward(); distanceR = distanceRight();} // move forward if you are against the wall of the obstacle
          turnRight(); _delay_ms(900);
          while(distanceF >= 15){ forward(); distanceF = distanceFront();} // move forward until front sensor sees a wall
          turnLeft(); _delay_ms(900);
          decision = 5;
      ...
      case 5: // move forward until we see a obstacle or wall
          ...
          if     ( distanceUF >= 15 && distanceL <= 15 ){decision = 3; break;} // obstacle avoid case left
          else if( distanceUF >= 15 && distanceR <= 15 ){decision = 4; break;} // obstacle avoid right
          else if( distanceF <= 15 && distanceL <= 15 ){decision = 2;  break;} // corner turn case for left
          else if( distanceF <= 15 && distanceR <= 15 ){decision = 1;  break;} //corner turn right
